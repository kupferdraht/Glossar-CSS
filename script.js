...
function initDB() {
    ...
    const request = indexedDB.open('GlossarDBCSS', 1);

    request.onerror = (event) => {
        console.error('IndexedDBCSS Fehler:', event.target.error);
    };
    ...
    request.onsuccess = (event) => {
        console.log('IndexedDBCSS erfolgreich initialisiert');
...
function exportEntries() {
    ...
    request.onsuccess = () => {
        ...
        const dateStr = now.toLocaleDateString('de-DE').replace(/\./g, '-'); // e.g., 24-07-2023
        const timeStr = now.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(/:/g, '-'); // e.g., 10-30-00
        const filename = `CSS-Glossar-Backup-${dateStr}-${timeStr}.json`;
